...
export enum PostConditionMode {
  /** `Allow` â€” Allow unspecified transfers */
  Allow = 0x01,
  /** `Deny` â€” Do not allow unspecified transfers */
  Deny = 0x02,
}
...
export enum AuthType {
  /** `Standard` (not sponsored) â€” The transaction is not sponsored. The sender will need to spend fees. */
  Standard = 0x04,
  /** `Sponsored` â€” The transaction is sponsored. The sponsor will spend fees on behalf of the sender. */
  Sponsored = 0x05,
}
...
export enum AddressHashMode {
  /** `SingleSigHashMode` â€” hash160(public-key), same as bitcoin's p2pkh */
  P2PKH = 0x00,
  /** Legacy `MultiSigHashMode` â€” hash160(multisig-redeem-script), same as bitcoin's multisig p2sh */
  P2SH = 0x01,
  /** `SingleSigHashMode` â€” hash160(segwit-program-00(p2pkh)), same as bitcoin's p2sh-p2wpkh */
  P2WPKH = 0x02,
  /** Legacy `MultiSigHashMode` â€” hash160(segwit-program-00(public-keys)), same as bitcoin's p2sh-p2wsh */
  P2WSH = 0x03,
  /** Non-Sequential `MultiSigHashMode` â€” hash160(multisig-redeem-script), same as bitcoin's multisig p2sh */
  P2SHNonSequential = 0x05,
  /** Non-Sequential `MultiSigHashMode` â€” hash160(segwit-program-00(public-keys)), same as bitcoin's p2sh-p2wsh */

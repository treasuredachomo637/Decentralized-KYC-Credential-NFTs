...
async function installPackages(pkgManager, packages) {
	if (!packages.length) {
		log(c.green("âœ”"), c.bold("All packages are already installed."));
		return;
	}
	log(c.cyan("â—¼"), c.bold("Installing packages..."));
	log(c.cyan("â—¼"), packages.join(", "));
	log();
...
async function updatePkgJsonScripts(pkgJsonPath, vitestScript) {
	...
	log(c.green("âœ”"), "Added \"test:browser\" script to your package.json.");
}
...
async function create() {
	log(c.cyan("â—¼"), "This utility will help you set up a browser testing environment.\n");
	...
	if (rootConfig) {
		...
		log(c.green("âœ”"), "Created a config file for browser tests:", c.bold(relative(process.cwd(), configPath)));
	}
	log();
	...
	if (installPlaywright) {
		...
		log(c.cyan("â—¼"), `Installing Playwright dependencies with \`${c.bold(command)} ${c.bold(allArgs.join(" "))}\`...`);
		log();
	...
	log(c.green("âœ”"), "Created example test file in", c.bold(relative(process.cwd(), exampleTestFile)));
	log(c.dim("  You can safely delete this file once you have written your own tests."));
	log();
	log(c.cyan("â—¼"), "All done! Run your tests with", c.bold(getRunScript(pkgManager)));
}
